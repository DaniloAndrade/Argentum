...
class MediaMovelSimplesTest extends Specification {
	...
	"Para uma sequencia simples de candles " >> {
		...
		val mms = new MediaMovelSimples(3,new IndicadorFechamento)
		...
		"o calculo para 4 serie deve ser 10.0/3 " >> {
			mms.calcula(4, serie).toDouble must beCloseTo(BigDecimal(10.0 / 3)
				.setScale(2,RoundingMode.HALF_UP).toDouble, 0.00001)
		}

		"o calculo para 5 serie deve ser 11.0/3 " >> {
			mms.calcula(5, serie).toDouble must beCloseTo(BigDecimal(11.0 / 3)
				.setScale(2,RoundingMode.HALF_UP).toDouble, 0.00001)
		}
		...
		"o calculo para 7 serie deve ser 13.0/3 " >> {
			mms.calcula(7, serie).toDouble must beCloseTo(BigDecimal(13.0 / 3)
				.setScale(2,RoundingMode.HALF_UP).toDouble, 0.00001)
		}
	...
	//TODO Criar testes com MediaMovelSimples com IndicadorAbertura
